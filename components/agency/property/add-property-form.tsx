...
export function AddPropertyForm({ onSuccess }: AddPropertyFormProps) {
  ...
  async function onSubmit(values: PropertyFormValues) {
    try {
      form.setValue("status", "listed");
      
      // Upload images to Supabase first and maintain description mapping
      ...
      if (images.length > 0) {
        ...
        try {
          // Collect all files with their original indices to maintain order
          const filesWithIndices: Array<{ file: File; index: number }> = [];
          images.forEach((img, index) => {
            if (img.file) {
              filesWithIndices.push({ file: img.file, index });
            }
          });
          
          // Upload all files at once
          if (filesWithIndices.length > 0) {
            const filesToUpload = filesWithIndices.map(item => item.file);
            const uploadedUrls = await uploadMultipleToSupabase(filesToUpload);
            
            // Create a map of uploaded URLs by their index in the filesWithIndices array
            const uploadedUrlsMap = new Map<number, string>();
            uploadedUrls.forEach((url, uploadIndex) => {
              const originalIndex = filesWithIndices[uploadIndex].index;
              uploadedUrlsMap.set(originalIndex, url);
            });
            
            // Map all images with their descriptions and uploaded URLs
            images.forEach((img, index) => {
              if (img.file && uploadedUrlsMap.has(index)) {
                // Use uploaded URL with its corresponding description
                propertyImages.push({
                  url: uploadedUrlsMap.get(index)!,
                  description: img.description || undefined,
                });
              } else if (!img.file) {
                // For images without files (shouldn't happen in normal flow)
                propertyImages.push({
                  url: img.url,
                  description: img.description || undefined,
                });
            ...
            // No files to upload, just use existing URLs
            images.forEach((img) => {
              propertyImages.push({
                url: img.url,
                description: img.description || undefined,
          ...
          toast.error("Failed to upload images. Please try again.", { id: "upload-images" });
      ...
      toast.loading("Adding property...", { id: "add-property" });
      
      const result = await addProperty(propertyData);

      if (result.success) {
        toast.success("Property added successfully!", { id: "add-property" });
        form.reset();
        setImages([]);
        onSuccess?.();
      } else {
        toast.error(result.message || "Failed to add property. Please try again.", { id: "add-property" });
      }
    } catch (error) {
      toast.error("Failed to add property. Please try again.", { id: "add-property" });
      console.error(error);
    }
  }
            ...
            {images.length === 0 && (
              <div className="border-2 border-dashed rounded-lg p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  No images selected. Click "Upload Images" to add property photos.
                </p>
              </div>
            )}
          </div>
